...
namespace world_info
{
    ...
    class DetectDoorSeg: public rclcpp::Node
    {
        public:

        explicit DetectDoorSeg(const rclcpp::NodeOptions &options = rclcpp::NodeOptions())
        : Node("door_seg", options),
       	// topics
        sub_cam(image_transport::create_subscription(this, "image_rect",
            std::bind(&DetectDoorSeg::onCamera, this, std::placeholders::_1),
            declare_parameter("image_transport", "raw", descr( {}, true)), rmw_qos_profile_sensor_data)),
        pub_door_seg(image_transport::create_publisher(this, "door_seg_detected"))
        {
            ...
            if (!has_parameter("door_seg_confidence_threshold"))
                declare_parameter("door_seg_confidence_threshold", CONFIDENCE_THRESHOLD);
            get_parameter("door_seg_confidence_threshold", CONFIDENCE_THRESHOLD);
            ...
            model = core.read_model(package_share_directory + "/weights/door_seg.onnx");
        }
        ...
        void onCamera(const sensor_msgs::msg::Image::ConstSharedPtr &msg_img)
        {
            ...
            sensor_msgs::msg::Image::SharedPtr img_msg = cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", img)
                .toImageMsg();
            pub_door_seg.publish(*img_msg.get());
        }

       	// const image_transport::CameraSubscriber sub_cam;
        const image_transport::Subscriber sub_cam;
        const image_transport::Publisher pub_door_seg;
...
RCLCPP_COMPONENTS_REGISTER_NODE(world_info::DetectDoorSeg)