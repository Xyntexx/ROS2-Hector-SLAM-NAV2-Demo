...
namespace world_info
{
    ...
    class DetectFirenet: public rclcpp::Node
    {
        public:

            explicit DetectFirenet(const rclcpp::NodeOptions &options = rclcpp::NodeOptions()): Node("firenet", options),
       	// topics
        sub_cam(image_transport::create_subscription(this, "image_rect",
            std::bind(&DetectFirenet::onCamera, this, std::placeholders::_1),
            declare_parameter("image_transport", "raw", descr( {}, true)), rmw_qos_profile_sensor_data)),
        pub_firenet(image_transport::create_publisher(this, "firenet_detected"))
        {
            ...
            if (!has_parameter("firenet_confidence_threshold"))
                declare_parameter("firenet_confidence_threshold", CONFIDENCE_THRESHOLD);
            get_parameter("firenet_confidence_threshold", CONFIDENCE_THRESHOLD);
            ...
            model = core.read_model(package_share_directory + "/weights/firenet_seg.onnx");
        }
        ...
        void onCamera(const sensor_msgs::msg::Image::ConstSharedPtr &msg_img)
        {
            ...
            sensor_msgs::msg::Image::SharedPtr img_msg = cv_bridge::CvImage(std_msgs::msg::Header(), "bgr8", img)
                .toImageMsg();
            pub_firenet.publish(*img_msg.get());
        }

       	// const image_transport::CameraSubscriber sub_cam;
        const image_transport::Subscriber sub_cam;
        const image_transport::Publisher pub_firenet;
...
RCLCPP_COMPONENTS_REGISTER_NODE(world_info::DetectFirenet)